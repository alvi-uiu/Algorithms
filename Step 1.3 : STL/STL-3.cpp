...
void ExplainAlgorithms() {
  ...
  cout << endl << "max element : " << el_ << endl;
  // sum i to j
...
void ExplainAlgorithms_part2() {
  ...
  cout << endl << "lowerBound idx: " << it - arr;

  auto it_ = lower_bound(vec.begin(), vec.end(), x);

  cout << endl << "lowerBoundVec idx : " << it_ - vec.begin();
...
bool ExplainComparator(
    int i,
    int j) { // comparator only compares two elements and returns true or false
